...
DllExport double *hilbertr_fir(double *ts, int len)
{
	...
	/* copy work to h, removing the group delay hlen/2 */
	work += hlen/2;
 ...
 */
static void spfilt(double *b, double *a, int lb, int la, double *x, int n)
{
	int ii, jj;
	double *px, *py;
	double sum;
	...
	for ( ii = 0 ; ii < n ; ii++ ) {
		...
		x[ii] = sum;
		if ( lb > 0 )
			memmove(px+1, px, (lb-1)*sizeof(*px));
		if ( la > 0 ) {
			memmove(py+1, py, (la-1)*sizeof(*py));
			py[0] = sum;
		}
	}
	if ( lb >0 )
